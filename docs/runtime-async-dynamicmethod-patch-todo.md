...
| 8 | [ ] | blocked | Add focused regression coverage for pair lifetime, forced finalization/reuse, failure cleanup, portable entrypoints, profiler lifecycle, runtime gating, and implementation-flag semantics. | Collectible-lifetime test was designed, but `.cs` transformation in the transaction worktree is blocked because hostfxr cannot resolve the repo-local SDK there. |  |
